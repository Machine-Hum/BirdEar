...
#from keras.layers import BatchNormalization, Bidirectional, LSTM, SimpleRNN, GRU
...
x_train = tf.keras.preprocessing.sequence.pad_sequences(xtrain, maxlen=10) # shape (896, 10,128)
x_val = tf.keras.preprocessing.sequence.pad_sequences(xval, maxlen=10) #(299,10,128)
...
model.add(tf.keras.layers.Conv1D(64, 3, activation='relu', input_shape=(10, 128)))
model.add(tf.keras.layers.Conv1D(64, 3, activation='relu'))

#model.add(tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(20, return_sequences=True), input_shape=(10, 1)))
#model.add(tf.keras.layers.Bidirectional(tf.keras.layers.SimpleRNN(100, return_sequences = True, activation="tanh")))
...
callbacks_list = [
    ...
    tf.keras.callbacks.EarlyStopping(monitor='acc',mode='auto', verbose=1, min_delta=0.01, patience=10) ]
